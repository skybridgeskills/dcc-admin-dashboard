...
export const getCredentialLinks: PayloadHandler = async (req, res) => {
  ...
  try {
    const credential = await payload.findByID({ id, collection: 'credential', depth: 3, showHiddenFields: true });
    ...
    const fetchResponse = await fetch(`${coordinatorUrl}/exchange/setup`, {
      ...
      headers: {
        ...
        'Authorization': `Bearer ${getTenant(credential.tenant).token}`
      },
      body: JSON.stringify({
        data: [{ vc: builtCredential, retrievalId: id }],
        tenantName: credential.tenant,
      }),
    });
    const json = await fetchResponse.json();

    if (json.code === 404) {
      throw Error("Coordinator could not find tenant: " + credential.tenant)
    }
    if (json.code === 401) {
      throw Error("Coordinator could not find auth header")
    }
    console.log(json)
